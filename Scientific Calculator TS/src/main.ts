...
const display = <htmlTags>document.getElementById('display');
const degToRadVal = <htmlTags>document.querySelector('.deg')
const memoryClearBtn = <htmlTags>document.getElementById('mc');
const memoryRecallBtn = <htmlTags>document.getElementById('mr');
const trigoFunc = <htmlTags>document.getElementById('trigo_func');
const invTrigoFunc = <htmlTags>document.getElementById('inverse_trigo_func');
const hypTrigoFunc = <htmlTags>document.getElementById('hyp_trigo_func');
const hypeInvTrigoFunc = <htmlTags>document.getElementById('hyp_inverse_trigo_func');
let memory: number[] = []
let flag: boolean = true
let degToRad: boolean = true
let fixedToExponential: boolean = true
let invTrigo: boolean = true
let hypeTrigo: boolean = true



function expoValue(num: number, sup: number): string {
    return Math.pow(num, sup).toString();
}

function onlyVal(val: number): string | void {
...
function toggleBtn(curr: string, next: string): void {
    const elementToHide = document.getElementsByClassName(curr) as HTMLCollectionOf<HTMLElement>
    const elementToShow = document.getElementsByClassName(next) as HTMLCollectionOf<HTMLElement>

    const hideElements = (elements: HTMLCollectionOf<HTMLElement>): void => {
    ...
    const showElements = (elements: HTMLCollectionOf<HTMLElement>): void => {
        for (let i: number = 0; i < elements.length; i++) {
            if (elements[i] instanceof HTMLElement) {
...
function factorial(): void {
    if (display === null) return
    let num: number = Number(display.value);
    if (num === 0 || num === 1) {
        display.value = "1";
    }
    else if (num > 1) {
...
function splitValues(symbol: string): number[] | void {
    if (display === null) return;
...
function yrootx(): string | void {
    if (display === null) return
    const nums: number[] | void = splitValues('√')
    if (nums != null) {
...
function getBaseLog() {
    if (display === null) return
    let nums: number[] | void = splitValues('ylog');
    if (nums != null) {
        display.value = (Math.log(nums[0]) / Math.log(nums[1])).toString();
    }
}

function enableMemoryClearAndRecall(): void {
    if (memoryRecallBtn === null || memoryClearBtn === null) return
...
function toggleTrigoFunc(): void {
    if (trigoFunc === null || invTrigoFunc === null || hypTrigoFunc === null || hypeInvTrigoFunc === null) return;
...
function calculateExpression(statement: string): void {
    if (display === null) {
        return
    }
    try {
...
function enterValue(enteredValue: string): void {
    if (display === null) return
    clearDisplay()
    switch (enteredValue) {
        //Deg and F-E function
        case 'deg': {
            if (degToRadVal === null) break;
            if (degToRad) {
